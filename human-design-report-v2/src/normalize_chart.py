...
CROSS_START_PATTERNS = [
    r"Right\s+Angle\s+Cross",
    r"Left\s+Angle\s+Cross",
    r"Juxtaposition\s+Cross",
]
...
def _extract_mhd_core_fields(text: str) -> dict[str, Optional[str]]:
    ...
    for index, line in enumerate(lines):
        ...
        if "TYPE" in upper and "AUTHORITY" in upper and index + 1 < len(lines):
            ...
            if " - " in combined:
                ...
                result["type"] = _clean_value(combined)
        ...
        if upper == "PROFILE":
            ...
            for value in next_values:
                if re.match(r"^\d/\d\b", value):
                    result["profile"] = _clean_value(value)
                    break
        ...
        if upper == "STRATEGY":
            ...
            for value in next_values:
                if not re.match(r"^\d/\d\b", value):
                    result["strategy"] = _clean_value(value)
                    break

        if "LIFE THEME" in upper or "INCARNATION CROSS" in upper:
            if index + 1 < len(lines):
                result["incarnation_cross"] = _clean_incarnation_cross(lines[index + 1])

        if upper == "DEFINITION":
            if index + 1 < len(lines):
                result["definition"] = _clean_value(lines[index + 1])
...
def _extract_named_planet_block(text: str, section: str) -> Dict[str, Optional[str]]:
    ...
    for planet in PLANETS:
        match = re.search(
            rf"(?i)\b{re.escape(planet)}\b\s*[:\-]\s*(\d{{1,2}}\.\d)",
            block,
        )
        if match:
...
def _extract_centers(text: str) -> Dict[str, Optional[str]]:
    ...
    for name in known:
        match = re.search(
            rf"(?im)^\s*{re.escape(name)}\s*[:\-]\s*(Defined|Undefined|Open)\b",
            text,
        )
...
def normalize_chart_data(raw_text: str) -> ChartData:
    regex_fields = {
        key: _extract_field_line(raw_text, labels)
    ...
    chart_dict["incarnation_cross"] = _clean_incarnation_cross(
        chart_dict.get("incarnation_cross")
    )
...
"""
    ...
    parsed = ChartData(**json.loads(json_str))
    parsed.incarnation_cross = _clean_incarnation_cross(parsed.incarnation_cross)
